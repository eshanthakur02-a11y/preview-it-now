  ...
  return (
    <>
      <button
        ...
        className="fixed right-5 bottom-5 z-50 h-12 w-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-lg ring-1 ring-primary/40"
        style={{ boxShadow: "var(--shadow-elegant, 0 10px 30px -10px rgba(0,0,0,.35))" }}
      >
        {open ? <X className="h-5 w-5" /> : <MessageCircle className="h-5 w-5" />}
      </button>

      {open && (
        <div className="fixed right-5 bottom-20 z-50 w-[380px] max-w-[calc(100vw-2.5rem)] h-[560px] max-h-[calc(100vh-7rem)] surface flex flex-col overflow-hidden animate-fade-up shadow-lg">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-background">
            {messages.length === 0 && (
              <div className="text-sm text-muted-foreground space-y-3">
                <p className="text-foreground/80">Hi — ask me anything. Try:</p>
                <ul className="space-y-1.5">
                  {[
                    ...
                    <li key={q}>
                      <button
                        onClick={() => send(q)}
                        className="text-left text-foreground hover:border-primary/40 hover:bg-accent/60 text-xs px-3 py-2 rounded-lg bg-card border border-border w-full transition-colors"
                      >
                        {q}
                      </button>
                    </li>
          ...
          <form onSubmit={handleSubmit} className="border-t border-border p-3 bg-card">
            <div className="flex items-end gap-2">
              <textarea
                ...
                className="flex-1 resize-none bg-background border border-border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring/30 focus:border-ring max-h-32 transition"
              />
