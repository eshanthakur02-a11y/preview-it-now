    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1 tabular-nums">{rows.length} record{rows.length === 1 ? "" : "s"}</p>
        </div>
        {canCreate && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                {columns.map((c) => <th key={String(c.key)} className="text-left px-4 py-3 font-semibold border-b border-border">{c.label}</th>)}
                {canDelete && <th className="w-12 border-b border-border"></th>}
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={columns.length + 1} className="px-4 py-12 text-center text-sm text-muted-foreground">No records yet</td></tr>
              ) : rows.map((row, i) => (
                <tr key={row.id} className={`border-t border-border transition-colors ${i % 2 === 1 ? "bg-muted/20" : ""} hover:bg-accent/40`}>
                  {columns.map((c) => (
                    <td key={String(c.key)} className="px-4 py-3 text-foreground/90">
                      {c.render ? c.render(row) : String((row as Record<string, unknown>)[c.key as string] ?? "—")}
                    </td>
                  ))}
                  {canDelete && (
                    <td className="px-2 py-2">
                      <Button size="icon-sm" variant="ghost" onClick={() => handleDelete(row.id)} aria-label="Delete">
