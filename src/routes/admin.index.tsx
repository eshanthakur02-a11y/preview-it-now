...
function AdminDashboard() {
  ...
  return (
    <>
      ...
      {empty && (
        <div className="surface p-6 mb-6 border-dashed">
          <h3 className="font-semibold mb-1 text-sm">Let's set up your school</h3>
          ...
          <div className="flex flex-wrap gap-2">
            <Link to="/admin/classes" className="text-xs px-3 py-1.5 rounded-md bg-primary/10 text-primary border border-primary/25 hover:bg-primary/15 transition-colors font-medium">+ Add classes</Link>
            <Link to="/admin/teachers" className="text-xs px-3 py-1.5 rounded-md bg-chart-2/10 text-chart-2 border border-chart-2/25 hover:bg-chart-2/15 transition-colors font-medium">+ Add teachers</Link>
            <Link to="/admin/students" className="text-xs px-3 py-1.5 rounded-md bg-success/10 text-success border border-success/25 hover:bg-success/15 transition-colors font-medium">+ Add students</Link>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <div className="surface p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold inline-flex items-center gap-2"><TrendingUp className="h-4 w-4 text-primary" /> Attendance · last 14 days</h3>
            <Link to="/admin/attendance" className="text-xs text-primary hover:underline underline-offset-4 font-medium">View register →</Link>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={attendanceTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: "var(--color-popover)", border: "1px solid var(--color-border)", borderRadius: 10, fontSize: 12 }} formatter={(v) => `${v}%`} />
              <Area type="monotone" dataKey="pct" stroke="var(--color-primary)" strokeWidth={2} fill="url(#dashAtt)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="surface p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold inline-flex items-center gap-2"><Wallet className="h-4 w-4 text-chart-2" /> Fee collection · last 6 months</h3>
            <Link to="/admin/fees" className="text-xs text-primary hover:underline underline-offset-4 font-medium">Fee structure →</Link>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={feeTrend} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: "var(--color-popover)", border: "1px solid var(--color-border)", borderRadius: 10, fontSize: 12 }} formatter={(v) => money(Number(v))} />
      ...
      <div className="surface p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold inline-flex items-center gap-2"><Megaphone className="h-4 w-4 text-primary" /> Latest announcements</h3>
          <Link to="/admin/announcements" className="text-xs text-primary hover:underline underline-offset-4 font-medium">Manage →</Link>
        </div>
            ...
            {announcements.map((a) => (
              <div key={a.id} className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {a.body && <div className="text-xs text-muted-foreground line-clamp-2 mt-0.5 leading-relaxed">{a.body}</div>}
                  <div className="text-[10px] text-muted-foreground/70 mt-1 tabular-nums">{fmt(a.created_at)}</div>
                </div>
              </div>
            ))}
          </div>
...
function RecentList({
  ...
  return (
    <div className="surface p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold inline-flex items-center gap-2"><Icon className="h-4 w-4 text-muted-foreground" /> {title}</h3>
        <Link to={link} className="text-xs text-primary hover:underline underline-offset-4 font-medium">View all →</Link>
      </div>
      {items.length === 0 ? (
        ...
        <div className="divide-y divide-border">
          {items.map((it) => (
            <div key={it.id} className="flex items-center gap-3 py-2.5">
              <div className="h-8 w-8 rounded-full flex items-center justify-center text-[11px] font-semibold shrink-0 bg-primary/10 text-primary ring-1 ring-inset ring-primary/20">
              ...
              <span className="text-[11px] text-muted-foreground shrink-0 tabular-nums">{it.meta}</span>
            </div>
          ))}
        </div>
      )}
...
function QuickAction({ to, label, icon: Icon }: { to: string; label: string; icon: typeof Users }) {
  return (
    <Link to={to} className="surface p-4 flex items-center gap-3 hover:border-primary/40 hover:bg-accent/40 transition-colors group">
      <div className="h-9 w-9 rounded-lg flex items-center justify-center shrink-0 bg-primary/10 text-primary ring-1 ring-inset ring-primary/20 group-hover:bg-primary/15 transition-colors">
